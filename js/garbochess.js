...
// Suggesting Moves
//
function SuggestMoves(finishMoveCallback, maxPly, finishPlyCallback) {
    var lastEval;
    var alpha = minEval;
    var beta = maxEval;
    ...
    for (i = 1; i <= maxPly && g_searchValid; i++) {
        ...
        if (finishPlyCallback != null) {
            finishPlyCallback(bestMoves, (new Date()).getTime() - g_startTime, i);
        }
    }

    if (finishMoveCallback != null) {
        finishMoveCallback(bestMoves, (new Date()).getTime() - g_startTime, i - 1);
    }
}
//
// Searching code
...
self.onmessage = function (e) {
	...
	else if (e.data.match("^suggest") == "suggest") {
		splitBuf = e.data.split(' ');
        g_timeout = parseInt(splitBuf[1], 10);
        SuggestMoves(null, 99, null);
	}
	else {
